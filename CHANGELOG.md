...
- 更新封面提取逻辑，确保从HDR源提取的封面颜色一致性。
- 修改封面文件命名为"rendered-cover.jpg"，提升可读性。
- 调整错误提示信息，确保用户理解更清晰。
- 在Media3RenderEngine中新增HDR到SDR的色调映射设置，优化视频导出效果。
- 引入新的工具脚本color_diff_probe.py，用于比较源帧与导出封面的颜色差异。
- 新增`AfterFrameSheet`通用底部弹窗与`AfterFrameConfetti`礼花动画组件，统一应用内弹窗样式。
- 新增`onDeviceBadge`本地化词条适配中英文展示。
- 重构个人页面布局、列表样式与本地工作区卡片UI。
- 将应用更新、清除缓存、语言选择、关于页面等原有弹窗替换为新的统一组件。
- 优化`AppUpdateCard`组件，支持嵌入式展示并调整UI细节。
- 优化测试文件，替换滚动查找逻辑并新增关于页面测试用例。

## 1.2.0 - 2026-08-27

- 重构「创作」页：去掉与主卡片重复的 Studio 入口和不可点的提示卡，只保留一张导入素材主卡片。
- 首页按 Dark / Memory / Minimal 方向升级，拆成 `home/` 下的独立组件：`MemoryGlowBackground` 用极低透明度的深绿光晕与同心光圈替代纯黑背景，8 秒呼吸一次，切走标签页或系统开启「减少动态效果」时自动停住；`CreateMemoryCard` 把「新的记忆 / 开始创作 / 导入视频」收成首页唯一主 CTA；`CreationFlow` 把视频 / Live 图 / 拼图用一条细连接线串成「选择片段 → 让瞬间流动 → 组合新的记忆」的创作流程。
- 首页新增「最近创作」横向列表，直接读取 `listExports()` 的真实作品封面，点按进入预览，「查看全部」跳到作品页；没有作品时整块隐藏，不出现空状态占位。
- 首页氛围光晕改为全屏绘制、安全区只作用于内容，刘海和挖孔屏不再在状态栏位置切出一条硬边并截断光圈。
- 「导入视频」不再撑满主卡片：改为包裹文字的胶囊按钮，箭头收进深色圆盘，呼应 Logo 的「亮绿底 + 深色光圈」。
- 视频 / Live 图 / 拼图三个入口在静止状态就带品牌绿：低透明度绿底、发丝级绿描边和绿色图标，按下时整体提亮并加光晕。
- 作品相关文案不再用「余帧」当作品量词：标题改为「我的作品」，并同步调整创建、删除、Studio 上限和关于页的说法；Logo 下方的品牌名「余帧」保留。
- 三级正文色提到 `#7B7E85`，让 11px 说明文字在深色背景上达到 WCAG AA 对比度。
- AfterFrame Studio 单帧编辑把「选择封面」和「瞬间时间轴」两套胶片条收成 `LiveMomentEditor` 一条轴：封面针与 LIVE 范围共用同一胶片，候选时刻仍可一键跳转。
- 拼图工具去掉重复的封面芯片、时间读数和第二套说明；素材轨点选当前段，封面滑杆与「裁剪」共用这一段，点按不再直接弹出裁剪面板。
- 生成只保留顶栏「导出」，去掉底部重复的生成按钮。
- 实况播放改为主流 Live 图手感：播放时画面缓出到 1.08 倍（`LivePlaybackScale`），结束或再次点击后回到 1.0，并停在封面静帧而不是最后一帧。编辑预览、全屏预览和作品预览共用同一套缩放。
- 增加创作页单一入口、首页安全区与光晕铺满、最近创作真实数据、单帧单轴和实况缩放回归测试。

## 1.1.0 - 2026-08-22

- Studio 移除素材后的提示改为自制玻璃条：保留「撤销」，4 秒后自动消失，不再使用会常驻的 Material SnackBar。
- 修复导入 Live 图后点「导出」既不成功也不失败的问题：抽出的 Live MP4 是 `file://` 缓存，原先在主线程用 ContentResolver 探测音轨会卡死；现改为文件路径探测、按真实样本时长夹取裁剪窗，音轨不可用或 Transformer 失败时回退为无声导出。
- 单帧封面条允许部分抽帧失败；导出不再因为封面条为空而静默忽略点击，顶栏导出按钮在处理中会显示进度。
- 修复 3 格 Live 拼图（含 Live 图）导出失败：Studio 预览会占满解码器，再开三路 Media3 会 `NO_MEMORY`。导出前先释放预览；模拟器或解码器耗尽时改为先合成两格、再叠第三格。
- 素材库同时收录普通视频和 Android Live 图（Motion Photo / 动态照片）。Google Motion Photo 1.0、MicroVideo 以及三星/小米等 JPEG 尾随 MP4 会被抽出为可播放源；HEIC Live 图在容器暴露视频轨时以 MediaExtractor 复用。
- 选择页增加「全部 / 视频 / Live」筛选、LIVE 徽章、有序已选托盘（可预览、移除、长按排序），视频与 Live 图可混选进入同一张 Live 单帧或 Live 拼图。
- AfterFrame Studio 可在未满 3 段时继续添加素材：单帧显示添加邀请，拼图轨提供 ＋ 入口；已编辑片段的裁剪和 Smart Crop 在身份不变时保留。
- 解析出的 Live 动态缓存在 `cacheDir/afterframe/motion_sources`，随临时缓存一并清理；缩略图优先使用静态主图。
- Android 13+ 同时请求 `READ_MEDIA_VIDEO` 与 `READ_MEDIA_IMAGES`，并更新首页、权限、隐私与拼图轨文案。
- 修复 Live 静图（常为 1080p）与同比例 720p 视频混排时上大下小、两侧黑边的问题：同比例家族改为等格缩放，预览与导出按完整取景窗均匀缩小到格子。
- Live 拼图封面改为每段独立选择；暂停预览各格显示各自封面，导出静图按各段封面合成。公共 Live 时长不超过最短素材（及最短有效裁切）。
- 时间码按 0.1 秒四舍五入，避免 2.49s 显示成 2.4s 而片段标签显示 2.5s。

## 1.0.0 - 2026-08-01

- 重构“我的”页信息架构：移除会制造账号编辑预期的固定人形头像和“记忆收藏家”假 ID，改为明确不可交互的 AfterFrame 本地工作区品牌卡；说明无需账号且素材、创作数据留在本机。
- 取消用“设置”统括全部入口，按主流产品语义拆分为“偏好设置 / 作品与导出 / 存储与隐私 / 支持与关于”，使可修改项、导航、信息说明、清理操作和支持动作各归其位。
- 完整实现“我的”页面：作品相册可直达作品页，导出画质与 Live 容器展示当前 Media3/Canvas First 的真实能力边界，“关于余帧”展示运行时版本、ABI、隐私摘要和 Flutter 开源许可。
- 增加临时缓存统计与确认清理。清理范围严格限制为 `cacheDir/afterframe` 下的缩略图、抽帧和未完成导出，不删除相册作品、作品索引、持久封面或分享 MP4；清理后同步失效 Flutter 内存路径缓存。
- 补充“我的”页面交互与缓存清理回归测试，并统一中英文文案。
- 将 App 语言改为与其他设置一致的单行卡片，点击后在底部选择语言；“我的”页每次重新进入都会刷新缓存统计，避免常驻 `IndexedStack` 展示启动时旧值。
- 将原生缓存范围抽离为 `TemporaryCacheManager` 并增加边界测试，验证只清理 `cacheDir/afterframe`，保留其他插件缓存与 `filesDir/afterframe` 持久作品。
- 新增独立“隐私与数据”入口，明确本地处理、更新联网、系统相册/私有副本/临时缓存边界，以及卸载后的数据归属。
