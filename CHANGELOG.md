...
- 将独立 Motion Canvas 控制器、同步预览、逐段裁剪、排序、模板、风格、转场和 Media3 导出嵌入 Studio，并统一为 Live 单帧的预览卡片与玻璃工具面板视觉语言。
- 重构 Live 拼图为 9:16 宽度优先的自适应画布：按素材宽高比评估候选分栏，默认使用完整画面最大内接矩形，禁止隐式放大裁剪与非等比拉伸。
- 素材支持在画布留白区域内直接拖动位置、双击复位；完整画面的归一化矩形通过 MethodChannel 传给 Media3，保证编辑预览、沉浸预览与成片使用同一几何参数。
- “动态记忆画布”工具面板支持下滑进入沉浸式全屏 Live 预览，并可上滑返回编辑；预览期间隐藏系统栏和编辑选中态。
