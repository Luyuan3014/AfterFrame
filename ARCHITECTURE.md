...
- Studio 共享顶栏、导出反馈、预览卡片和玻璃面板视觉层；拼图业务状态仍由独立 `MotionCanvasController` 管理，不与单帧时间轴状态混写。
- 选择 1 段素材时拼图模式不可用；选择 2–3 段时默认进入拼图，并允许在 Studio 内无损切回单帧。
- 同时支持 1–3 路视频。
- 每一路使用自己的裁切区间和焦点坐标。
- 输出固定为竖屏 1080×1920；Flutter 根据素材宽高比对横分栏、竖分栏、左主画面和上主画面候选计算无裁剪占用率，选择完整画面面积最大的方案。
- 每个模板槽位只定义可用区域，素材按原始比例求最大内接矩形；宽度已满时保留自然高度和设计留白，不再为了填满槽位放大裁边。
- 完整素材矩形使用 0–1 归一化 `x/y/width/height` 传给 Android；Media3 使用 `LAYOUT_SCALE_TO_FIT`，编辑预览、沉浸预览、成片及成片预览均禁止隐式裁剪和拉伸。
- 手动拖动只改变完整素材在剩余留白中的位置，双击恢复居中；没有剩余空间的轴向不会产生视觉位移。
- 编辑预览和沉浸式预览复用同一个 `MotionCanvasRenderer`；工具面板下滑进入预览，上滑退出，预览状态不改变导出参数。
